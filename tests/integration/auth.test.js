...
const { roleRights, DEFINED_ROLES } = require('../../src/config/roles');
...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/logout', () => {
    test('should return 204 if refresh token is valid', async () => {
      ...
      const refreshToken = tokenService.generateToken(userOne, expires, TOKEN_TYPES.REFRESH);
      await tokenService.saveToken(refreshToken, userOne._id, expires, TOKEN_TYPES.REFRESH);
    ...
    test('should return 404 error if refresh token is blacklisted', async () => {
      ...
      const expires = moment().add(config.jwt.refreshExpirationDays, 'days');
      const refreshToken = tokenService.generateToken(userOne, expires, TOKEN_TYPES.REFRESH);
      await tokenService.saveToken(refreshToken, userOne._id, expires, TOKEN_TYPES.REFRESH, true);
  ...
  describe('POST /v1/auth/refresh-tokens', () => {
    test('should return 200 and new auth tokens if refresh token is valid', async () => {
      ...
      expect(dbRefreshTokenDoc).toMatchObject({ type: TOKEN_TYPES.REFRESH, user: userOne._id, blacklisted: false });
    ...
    test('should return 401 error if refresh token is signed using an invalid secret', async () => {
      await insertUsers([userOne]);
      const expires = moment().add(config.jwt.refreshExpirationDays, 'days');
      const refreshToken = tokenService.generateToken(userOne, expires, TOKEN_TYPES.REFRESH, 'invalidSecret');
      await tokenService.saveToken(refreshToken, userOne._id, expires, TOKEN_TYPES.REFRESH);
    ...
    test('should return 401 error if refresh token is expired', async () => {
      ...
      const refreshToken = tokenService.generateToken(userOne, expires);
      await tokenService.saveToken(refreshToken, userOne._id, expires, TOKEN_TYPES.REFRESH);
  ...
  describe('POST /v1/auth/reset-password', () => {
    test('should return 204 and reset the password', async () => {
      ...
      const resetPasswordToken = tokenService.generateToken(userOne, expires, TOKEN_TYPES.RESET_PASSWORD);
      await tokenService.saveToken(resetPasswordToken, userOne._id, expires, TOKEN_TYPES.RESET_PASSWORD);
    ...
    test('should return 401 if reset password token is blacklisted', async () => {
      ...
      const expires = moment().add(config.jwt.resetPasswordExpirationMinutes, 'minutes');
      const resetPasswordToken = tokenService.generateToken(userOne, expires, TOKEN_TYPES.RESET_PASSWORD);
      await tokenService.saveToken(resetPasswordToken, userOne._id, expires, TOKEN_TYPES.RESET_PASSWORD, true);
  ...
  describe('POST /v1/auth/verify-email', () => {
    test('should return 204 and verify the email', async () => {
      ...
      const verifyEmailToken = tokenService.generateToken(userOne, expires);
      await tokenService.saveToken(verifyEmailToken, userOne._id, expires, TOKEN_TYPES.VERIFY_EMAIL);
    ...
    test('should return 401 if verify email token is blacklisted', async () => {
      ...
      const expires = moment().add(config.jwt.verifyEmailExpirationMinutes, 'minutes');
      const verifyEmailToken = tokenService.generateToken(userOne, expires);
      await tokenService.saveToken(verifyEmailToken, userOne._id, expires, TOKEN_TYPES.VERIFY_EMAIL, true);
...
describe('Auth middleware', () => {
  ...
  test('should call next with unauthorized error if access token is generated with an invalid secret', async () => {
    await insertUsers([userOne]);
    const expires = moment().add(config.jwt.accessExpirationMinutes, 'minutes');
    const accessToken = tokenService.generateToken(userOne, expires, TOKEN_TYPES.ACCESS, 'invalidSecret');
  ...
  test('should call next with unauthorized error if access token is expired', async () => {
    await insertUsers([userOne]);
    const expires = moment().subtract(1, 'minutes');
    const accessToken = tokenService.generateToken(userOne, expires, TOKEN_TYPES.ACCESS);
    const req = httpMocks.createRequest({ headers: { Authorization: `Bearer ${accessToken}` } });
    const next = jest.fn();
